...
public class BookingController {
    ...
    @PostMapping
    public Booking addBooking(@RequestBody @Valid BookingIncomingDto bookingIncomingDto, @RequestHeader("X-Sharer-User-Id") Long booker) throws ItemNotAvailableException, ItemNotFoundException, IncorrectBookingException, UserNotFoundException {
        return bookingService.addBooking(bookingIncomingDto, booker);
    }

    @PatchMapping("/{bookingId}")
    public Booking setBookingStatus(@PathVariable Long bookingId, @RequestParam Boolean approved, @RequestHeader("X-Sharer-User-Id") Long owner) throws IncorrectOwnerException, IncorrectBookingException, ItemNotFoundException {
        return bookingService.setBookingStatus(bookingId, approved, owner);
    }

    @GetMapping("/{bookingId}")
    public Booking getBookingById(@PathVariable Long bookingId, @RequestHeader("X-Sharer-User-Id") Long owner) throws IncorrectOwnerException, BookingNotFoundException, ItemNotFoundException {
